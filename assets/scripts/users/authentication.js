...
import { MODES, getMode, processMode, setMode } from '../app/mode'
import { applyFlagOverrides, generateFlagOverrides } from '../app/flag_utils'
import {
  clearSignInData,
  rememberUserProfile,
  setSignInData
} from '../store/slices/user'
import { createNewStreetOnServer, fetchStreetFromServer } from '../streets/xhr'
...
export function doSignIn () {
  console.log('scripts/users/authentication doSignIn')
  ...
  if (newAuthEnabled) {
    console.log('showDialog SIGN_IN')
...
export function clearAllClientSignInData () {
  // console.log('clearAllClientSignInData')
...
function saveSignInDataLocally () {
  // console.log('saveSignInDataLocally ')
  const signInData = getSignInData()
  if (signInData) {
    // console.log('saveSignInDataLocally: ', signInData)
...
export async function loadSignIn () {
  // console.log('authentication loadSignIn')
  ...
  if (signInCookie && userIdCookie && refreshCookie) {
    // console.log('loadSignIn setzt signin in store')
    store.dispatch(
      setSignInData({
        token: signInCookie,
  ...
  try {
    if (signInData.token) {
      jwt.verify(signInData.token, process.env.JWT_SECRET);
    }
  } catch (error) {
  ...
  try {
    ...
    if (signInData && signInData.token && signInData.userId) {
      ...
      const decoded = jwt.verify(signInData.token, process.env.JWT_SECRET);
      // console.log('signInData.token decoded ', decoded)
      ...
      // console.log('signInData.userId ', signInData.userId)
    ...
    console.log('Error loadSignIn ', error)
  }
...
async function fetchSignInDetails (userId) {
  try {
    ...
    // console.log('fetchSignInDetails ', json)
...
function receiveSignInDetails (details) {
  console.log('receiveSignInDetails:' , details)
  const signInData = {
    ...getSignInData(),
    details
  }
